...
- Desktop focused lifecycle regression suite: 14 passed.
- Desktop full suite: 184 passed across 33 files.
- Desktop main and renderer typecheck: passed.
- Desktop production build: passed for version 1.2.14.
- Backend focused control-plane suite: 10 passed.
- Backend full suite: 499 passed and 20 skipped.
- Two pre-existing concurrency timing tests exceeded their narrow thresholds once under local full-suite load; both then passed in three consecutive focused reruns, and the second complete Backend run passed with zero failures.
- `openspec validate stabilize-companion-polling-lifecycle --strict`: passed.
