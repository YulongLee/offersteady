...
### Requirement: 管理员短信验证与会话安全
系统 SHALL 在签发管理会话前验证已授权管理员手机号和短信验证码，并 SHALL 对高风险动作要求近期短信验证证明。

#### Scenario: 缺少短信验证
- **WHEN** 用户未完成有效管理员手机号短信验证
- **THEN** 系统不签发管理会话

#### Scenario: 高风险操作的短信验证已过期
- **WHEN** 管理员执行财务调整或管理员授权操作且近期短信验证已过期
- **THEN** 系统要求重新短信登录并在验证成功前拒绝操作
...
### Requirement: 受控管理员生命周期
系统 SHALL 允许现有超级管理员通过独立后台为已注册用户授予预设管理员角色或停用管理员，并 SHALL NOT 允许自定义权限、停用自己或停用最后一名超级管理员。

#### Scenario: 添加其他管理员
- **WHEN** 已完成近期短信验证的超级管理员选择已注册用户、预设角色、填写原因并二次确认
- **THEN** 系统创建或轮换管理员授权，新管理员可使用授权手机号和短信验证码登录，并记录脱敏审计

#### Scenario: 普通管理员尝试授权
- **WHEN** 不具备 `admins.manage` 权限的管理员尝试添加管理员
- **THEN** 系统拒绝请求并记录权限拒绝审计

#### Scenario: 停用管理员
- **WHEN** 超级管理员停用另一名管理员
- **THEN** 系统提升授权版本、撤销目标管理会话并保留历史审计

#### Scenario: 破坏管理可恢复性
- **WHEN** 管理员尝试停用自己或最后一名有效超级管理员
- **THEN** 系统拒绝操作
