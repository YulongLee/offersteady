...
- [x] 1.1 Add the versioned multilingual locale registry, provider language codes, release tiers, RTL metadata, and shared protocol/domain types.
- [x] 1.2 Add the user default interview-language setting with authenticated read/write API, validation, migration, and persistence round-trip tests.
- [x] 1.3 Extend session creation/update schemas and repositories to persist the selected locale, preserve legacy defaults, and lock it after start.
- [x] 1.4 Add capability gating so only locales with provider access, prompt assets, and passing release checks can start production sessions.

## 2. Global Web experience

- [x] 2.1 Add default interview-language controls to the existing Settings page without changing the current layout or visual tokens.
...
- [x] 2.6 Keep the preparation-page language selector expanded, show English as the green default Production tier, and preserve the existing readiness flow.

## 3. Backend routing

- [x] 3.1 Route realtime ASR prewarm, ingestion, reconnect, and source-session cache identity through the authoritative locale registry.
- [x] 3.2 Route question detection/normalization, chat quick/detail/continuation, screenshot analysis, fallback, review, and export formatting through the session locale.
...
- [x] 4.1 Add independently versioned system/quick/detail/continuation/screenshot prompt assets for the ten production locales.
- [x] 4.2 Add the shared commercial interview answer contract to every locale prompt: concise spoken delivery, conclusion-first structure, evidence grounding, uncertainty labels, no fabricated experience, and AI-advice disclosure.
- [x] 4.3 Add beta prompt scaffolding and fail-closed checks so incomplete locales cannot silently use Chinese or English templates.
- [x] 4.4 Add locale-aware output validation and one bounded repair retry for materially wrong-language responses.
