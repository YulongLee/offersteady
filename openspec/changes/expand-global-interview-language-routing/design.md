...
沿用现有 `.interview-language-picker`、设置面板和 `live-language-badge` 样式，在准备页直接展开语言列表，让用户无需额外点击即可比较和选择语言；默认布局、颜色、间距和主要 CTA 不变。英语条目使用绿色的默认 Production 标识，其他条目显示“生产支持”或“Beta”标签；RTL 语言只增加方向属性和文本流适配，不重排页面结构。
