...
- 网页对正常前缀增长的 Partial revision 立即更新，不再通过字幕蓄水池或 Final 尾字动画换取表面平滑；后端与浏览器状态层共同维护“稳定前缀 + 有界可变尾部”，较短 revision 或修改稳定前缀的同长/更长 revision 均不得删除已见正文，渲染层按最长公共前缀隔离可变尾部，避免纠错时整段字幕被替换。
