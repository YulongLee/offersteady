...
- 网页对每个合法的新 Partial revision 立即采用供应商权威文本，不再通过字幕蓄水池、较短 Partial 保留或 Final 尾字动画换取表面平滑；渲染层按最长公共前缀隔离可变尾部，保证当前 revision 文本在同一 React render 中完整呈现，同时减少纠错时整段 DOM 抖动。
