...
- 为已收到的 Partial Transcript 增加有界、自适应的字幕蓄水池：首个字符立即出现，剩余新增文字依据最近 revision 间隔与库存深度动态出水，并按真实批次间隔在 `800–1000ms` 内追平；仅对前缀增长型 Final 的新增尾字执行 `150–250ms` 快速平滑，Final 纠错、后台页面和减少动态效果模式保持即时展示。
