...
const BASE = process.env.OFFERSTEADY_CAPTURE_BASE ?? 'http://127.0.0.1:5187';
const OUT = resolve('video/offersteady-web-commercial/public/textures');
const chromePath = process.env.CHROME_PATH ?? '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
await mkdir(OUT, {recursive: true});

const browser = spawn(chromePath, [
  '--headless=new', '--disable-gpu', '--no-first-run', '--no-default-browser-check',
  '--remote-debugging-port=0', '--user-data-dir=/tmp/offersteady-commercial-chrome', `${BASE}/`,
...
for (const page of pages) {
  ...
  if (page.name === 'library') {
    for (const [i,label] of ['简历','职位 JD','知识库'].entries()) {
