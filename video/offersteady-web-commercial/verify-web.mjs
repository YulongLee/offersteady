...
const browser = spawn(chromePath, [
  ...
  '--remote-debugging-port=0', '--user-data-dir=/tmp/offersteady-web-verification-chrome', `${BASE}/`,
...
for(const [width,height,mobile] of [[1280,900,false],[390,844,true]]){
 ...
 await command('Page.navigate',{url:BASE});await pause(1200);
 ...
 await evaluate(`document.querySelector('video').play()`);await pause(1000);
 results.push(await evaluate(`(()=>{const v=document.querySelector('video');return{viewport:innerWidth,documentWidth:document.documentElement.scrollWidth,duration:v.duration,videoWidth:v.videoWidth,videoHeight:v.videoHeight,currentTime:v.currentTime,paused:v.paused,muted:v.muted,error:v.error?.message??null}})()`));
 await evaluate(`new Promise(resolve=>{const v=document.querySelector('video');v.pause();v.onseeked=()=>resolve();v.currentTime=20.4;setTimeout(resolve,5000);})`);await pause(800);results.push(await evaluate(`(()=>{const v=document.querySelector('video');return {seekTime:v.currentTime,readyState:v.readyState,seeking:v.seeking,networkState:v.networkState,decodedFrames:v.getVideoPlaybackQuality().totalVideoFrames,buffered:[...Array(v.buffered.length)].map((_,i)=>[v.buffered.start(i),v.buffered.end(i)])}})()`));await saveShot('preview-'+width);
}
