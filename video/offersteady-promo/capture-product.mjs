...
const BASE = process.env.OFFERSTEADY_CAPTURE_BASE ?? 'http://127.0.0.1:5181';
const OUT = resolve('video/offersteady-promo/public/textures');
const chromePath = process.env.CHROME_PATH ?? '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
await mkdir(OUT, {recursive: true});

const browser = spawn(chromePath, [
  '--headless=new', '--disable-gpu', '--no-first-run', '--no-default-browser-check',
  '--remote-debugging-port=0', '--user-data-dir=/tmp/offersteady-promo-chrome', `${BASE}/`,
...
const pages = [
  ...
  {name: 'live', path: '/app/interviews/demo/live', selectors: ['.live-top', '.conversation-monitor', '.answer-workspace', '.compact-question-bar']},
  {name: 'review', path: '/app/interviews/demo/review', selectors: ['.review-main', '.review-summary', '.review-timeline']},
  {name: 'library', path: '/app/library', selectors: ['.app-page', '.library-list']},
];
const layout = {};
