...
files=['offersteady-global-commercial.mp4','offersteady-global-commercial-nobgm.mp4','web/offersteady-global-web.mp4','web/offersteady-global-web-720.mp4']
results={}
for name in files:
 f=out/name
 ...
 raw=f.read_bytes(); atoms=[]; pos=0
 while pos+8<=len(raw):
  size,tag=struct.unpack('>I4s',raw[pos:pos+8]); atoms.append(tag.decode(errors='replace'))
  if size<8: break
  pos+=size
 results[name]={'bytes':f.stat().st_size,'sha256':hashlib.sha256(raw).hexdigest(),'faststart':atoms.index('moov')<atoms.index('mdat'),'streams':[{k:s[k] for k in ['codec_type','codec_name','width','height','r_frame_rate','duration','nb_frames','pix_fmt','sample_rate'] if k in s} for s in data['streams']], 'decodeExit':check.returncode,'decodeErrors':check.stderr.decode()}
y=read(out/files[1]);m=json.loads((p/'reference/sfx-measurements.json').read_text());sync=[]
for target,src in [(55,'transition-soft.mp3'),(165,'whoosh-fast.mp3'),(311,'transition-soft.mp3'),(379,'whoosh-fast.mp3'),(831,'click-camera.mp3'),(963,'transition-soft.mp3'),(1273,'whoosh-fast.mp3'),(1459,'click-camera.mp3'),(1534,'transition-soft.mp3'),(1899,'riser-rise.mp3'),(1944,'impact-deep-whoosh.mp3'),(2020,'shimmer-sparkle-sweep.mp3')]:
