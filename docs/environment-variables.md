...
- `OFFERSTEADY_PARTNER_PROGRAM_ENABLED`：合作伙伴计划总开关，默认 `false`；仅在推广中心已启用且结算 SOP 已确认后开启
- `OFFERSTEADY_PARTNER_COMMISSION_RATE_BPS`：佣金基点，首版 `2000`（20%）
- `OFFERSTEADY_PARTNER_COMMISSION_RULE_VERSION`：计佣规则版本，变更费率或窗口时递增
- `OFFERSTEADY_PARTNER_ELIGIBLE_ORDER_DAYS`：被推广用户注册后的订单计佣窗口，默认 `90`
- `OFFERSTEADY_PARTNER_REFUND_HOLD_DAYS`：佣金退款观察期，默认 `7`
- `OFFERSTEADY_PARTNER_MINIMUM_PAYOUT_CENTS`：最低结算金额（分），默认 `10000`
- `OFFERSTEADY_PARTNER_AGREEMENT_VERSION`：用户加入时必须接受的合作协议版本
- `OFFERSTEADY_PARTNER_PROJECTION_BATCH_SIZE`：独立推广任务每轮最多投影的支付订单数，默认 `200`
