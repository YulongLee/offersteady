...
class ChatService:
    ...
    def stream_automatic_answer_question(
        self, *, user_id: str, session_id: str, question: str, usage_id: str | None = None
    ) -> Iterator[dict]:
        """Stream the existing single-stage automatic answer without changing its prompt contract."""
        session = self.session_service.get_session(user_id=user_id, session_id=session_id)
        if session.status != "live":
            raise DomainRequestError("live-answer", "start-automatic-stream", "只有进行中的面试会话才能发起实时回答。", 400)
        self.session_service.touch_activity(user_id=user_id, session_id=session_id, force=True)
        now_ms = _now_ms()
        task_id = f"answer-{uuid4().hex}"
        billing_usage_id = usage_id or f"live-answer:{task_id}"
        ...
        self._log(logging.INFO, "chat.automatic_stream_started", task=task, session_id=session_id, question=question, retry_count=0)
        self.session_service.append_context(
            user_id=user_id,
            session_id=session_id,
            role="manual-question",
        ...
        material_context_text, material_assembly, material_provenance = self._assemble_material_context(session=session, retrieval=retrieval)
        system_prompt, prompt_config = self.prompt_template.load_system_prompt()
        history_entries = self.session_service.get_context_window(
            user_id=user_id,
            session_id=session_id,
            limit=self.settings.chat_max_history_entries,
        )
        prompt = self.prompt_builder.build(
            question=question,
            session_title=session.title,
        ...
        retrieval_response = self._to_retrieval_response(retrieval)
        yield {"type": "task-started", "task": current_task, "retrieval": retrieval_response}
        ...
        last_session_check_at_ms = stream_started_at_ms
        for attempt in range(self.settings.chat_retry_max_attempts + 1):
            try:
                if self._is_task_cancelled(current_task.task_id):
                    ...
                    return
                for provider_chunk in self.llm_gateway.stream_generate(question=question, prompt=prompt, attempt=attempt):
                    if self._is_task_cancelled(current_task.task_id):
                        cancelled = self.repository.get_task(current_task.task_id) or current_task
                        yield {"type": "cancelled", "task": cancelled}
                        return
                    now_ms = _now_ms()
                    if now_ms - last_session_check_at_ms >= 500:
                        last_session_check_at_ms = now_ms
                        if self.session_service.get_session(user_id=user_id, session_id=session_id).status != "live":
                            self.cancel_task(user_id=user_id, task_id=current_task.task_id)
                            cancelled = self.repository.get_task(current_task.task_id) or current_task
                            yield {"type": "cancelled", "task": cancelled}
                            return
                    if not provider_chunk.text:
                        continue
                    if first_token_at_ms is None:
                        first_token_at_ms = _now_ms()
                    chunk = ChatAnswerChunk(
                        sequence=len(chunks) + 1,
                        text=provider_chunk.text,
                        is_final=False,
                        provider_finish_reason=provider_chunk.provider_finish_reason,
                    )
                    chunks.append(chunk)
                    ...
                    yield {"type": "chunk", "task": current_task, "chunk": chunk}

                final_text = "".join(answer_parts).strip()
                if not final_text:
                    raise NonRetryableChatError(
                        "当前对话模型返回了无效结果，请稍后重试或检查模型配置。",
                        code="chat_provider_invalid_response",
                    )
                if chunks:
                    last = chunks[-1]
                    chunks[-1] = replace(last, is_final=True)
                usage = UsageReport(
                    prompt_tokens=max(1, len(prompt.rendered_prompt) // 4),
                ...
                if self.billing_service is not None:
                    self.billing_service.settle_usage(usage_id=billing_usage_id)
                yield {"type": "completed", "task": completed, "retrieval": retrieval_response}
                return
                ...
                self._log(
                    logging.WARNING,
                    "chat.automatic_stream_retrying",
                    task=current_task,
                    session_id=session_id,
                    question=question,
        ...
        yield {
            ...
            "partial_text": failed.answer_text,
        }
