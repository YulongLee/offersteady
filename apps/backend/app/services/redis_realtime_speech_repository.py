...
class RedisRealtimeSpeechRepository(InMemoryRealtimeSpeechRepository):
    ...
    def _snapshot(self) -> dict[str, object]:
        return {
            ...
            "receipts": [asdict(item) for bucket in self.frame_receipts.values() for item in bucket.values()],
            "transcripts": [asdict(item) for bucket in self.transcripts.values() for item in bucket.values()],
            "candidates": [asdict(item) for item in self.candidates.values()],
            "events": [asdict(item) for bucket in self.events.values() for item in bucket[-self._event_retention:]],
    ...
    def save_frame_receipt(self, receipt): return self._write(lambda: super(RedisRealtimeSpeechRepository, self).save_frame_receipt(receipt))
    def list_frame_receipts_for_session(self, *, session_id): return self._read(lambda: super(RedisRealtimeSpeechRepository, self).list_frame_receipts_for_session(session_id=session_id))
    def save_transcript(self, segment):
        stored = self._write(lambda: super(RedisRealtimeSpeechRepository, self).save_transcript(segment))
        ...
        return stored
    def get_transcript(self, session_id, segment_id): return self._read(lambda: super(RedisRealtimeSpeechRepository, self).get_transcript(session_id, segment_id))
    def list_transcripts_for_session(self, *, session_id): return self._read(lambda: super(RedisRealtimeSpeechRepository, self).list_transcripts_for_session(session_id=session_id))
    ...
    def save_event(self, event):
        stored = self._write(lambda: super(RedisRealtimeSpeechRepository, self).save_event(event))
    ...
    def list_events_for_session(self, *, session_id): return self._read(lambda: super(RedisRealtimeSpeechRepository, self).list_events_for_session(session_id=session_id))
    def get_session_activity_version(self, *, session_id): return self._read(lambda: super(RedisRealtimeSpeechRepository, self).get_session_activity_version(session_id=session_id))
