...
METRICS: dict[str, MetricDefinition] = {
    ...
    "answer_requests": MetricDefinition("answer_requests", "回答请求", "次", "interview", "sum", "面试场景中的文本回答模型调用数。", f"{BOUNDS} SELECT COUNT(*)::DOUBLE PRECISION AS value, COUNT(*)::BIGINT AS sample_count FROM ai_usage_records, bounds WHERE created_at_ms >= start_ms AND created_at_ms < end_ms AND session_id IS NOT NULL AND operation_kind = 'chat'"),
    "screenshot_answers": MetricDefinition("screenshot_answers", "截图回答", "次", "interview", "sum", "截图或视觉回答模型调用数。", f"{BOUNDS} SELECT COUNT(*)::DOUBLE PRECISION AS value, COUNT(*)::BIGINT AS sample_count FROM ai_usage_records, bounds WHERE created_at_ms >= start_ms AND created_at_ms < end_ms AND operation_kind = 'vision'"),
    ...
    "materials_ready": MetricDefinition("materials_ready", "资料可用", "份", "material", "sum", "自然日内完成处理并可使用的资料数。", f"{BOUNDS} SELECT COUNT(*)::DOUBLE PRECISION AS value, COUNT(*)::BIGINT AS sample_count FROM material_documents, bounds WHERE updated_at_ms >= start_ms AND updated_at_ms < end_ms AND status = 'ready' AND deleted_at_ms IS NULL"),
    "material_success_rate": MetricDefinition("material_success_rate", "资料成功率", "%", "material", "weighted_average", "自然日内发生最终状态变化的资料中可用资料比例。", f"{BOUNDS} SELECT CASE WHEN COUNT(*) = 0 THEN NULL ELSE COUNT(*) FILTER (WHERE status = 'ready') * 100.0 / COUNT(*) END::DOUBLE PRECISION AS value, COUNT(*)::BIGINT AS sample_count FROM material_documents, bounds WHERE updated_at_ms >= start_ms AND updated_at_ms < end_ms AND status NOT IN ('uploading', 'processing', 'deleted') AND deleted_at_ms IS NULL"),
    ...
    "ai_error_rate": MetricDefinition("ai_error_rate", "AI 异常率", "%", "quality", "weighted_average", "模型调用中非成功状态的比例。", f"{BOUNDS} SELECT CASE WHEN COUNT(*) = 0 THEN NULL ELSE COUNT(*) FILTER (WHERE status <> 'succeeded') * 100.0 / COUNT(*) END::DOUBLE PRECISION AS value, COUNT(*)::BIGINT AS sample_count FROM ai_usage_records, bounds WHERE created_at_ms >= start_ms AND created_at_ms < end_ms"),
    ...
    "answer_first_token_ms": MetricDefinition("answer_first_token_ms", "回答首字延迟", "ms", "quality", "weighted_average", "流式回答从请求开始到首个可见字符的平均延迟。", f"{BOUNDS} SELECT AVG(first_token_ms)::DOUBLE PRECISION AS value, COUNT(first_token_ms)::BIGINT AS sample_count FROM ai_usage_records, bounds WHERE created_at_ms >= start_ms AND created_at_ms < end_ms AND operation_kind = 'chat'", backfillable=False),
    "screenshot_first_token_ms": MetricDefinition("screenshot_first_token_ms", "截图回答首字延迟", "ms", "quality", "weighted_average", "截图视觉请求从开始到首个可见字符的平均延迟。", f"{BOUNDS} SELECT AVG(first_token_ms)::DOUBLE PRECISION AS value, COUNT(first_token_ms)::BIGINT AS sample_count FROM ai_usage_records, bounds WHERE created_at_ms >= start_ms AND created_at_ms < end_ms AND operation_kind = 'vision'", backfillable=False),
    "asr_final_latency_ms": MetricDefinition("asr_final_latency_ms", "ASR 最终延迟", "ms", "quality", "weighted_average", "最终语音分段从开始识别到最终转写完成的平均延迟。", f"{BOUNDS} SELECT AVG(final_latency_ms)::DOUBLE PRECISION AS value, COUNT(final_latency_ms)::BIGINT AS sample_count FROM ai_usage_records, bounds WHERE created_at_ms >= start_ms AND created_at_ms < end_ms AND operation_kind = 'speech'", backfillable=False),
