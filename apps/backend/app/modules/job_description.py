...
from app.schemas.material_upload import CompleteUploadRequest, CreatePastedJobDescriptionRequest, MaterialUploadCompletionResponse, UploadIntentRequest, UploadIntentResponse
...
descriptor = ModuleDescriptor(
    ...
    route_prefix="/api/v1/job-descriptions",
    mode="active",
    notes="JD uploads support OSS-backed file intents plus pasted-text creation.",
)
...
async def status(request: Request) -> ApiEnvelope[dict[str, str]]:
    return success_response(
        request=request,
        data={"status": "active", "feature": "job-description", "message": "Job-description upload pipeline is available in MVP upload mode."},
        timestamp=utc_now_iso(),
    )


@router.post("/upload-intents", response_model=ApiEnvelope[UploadIntentResponse])
async def create_job_description_upload_intent(
    ...
    authorize_global_feature(user_id=user_id, feature="resumeJd")
...
async def complete_job_description_upload(
    request_context: Request,
...
async def proxy_job_description_upload(
    request_context: Request,
...
@router.post("/text", response_model=ApiEnvelope[MaterialUploadCompletionResponse])
async def create_pasted_job_description(
    request_context: Request,
    request: CreatePastedJobDescriptionRequest,
    auth_context: AuthenticatedRequestContext | None = Depends(optional_authenticated_context),
    service: DocumentService = Depends(document_service),
) -> ApiEnvelope[MaterialUploadCompletionResponse]:
    user_id = resolve_owned_user_id(explicit_user_id=request.user_id, auth_context=auth_context)
    authorize_global_feature(user_id=user_id, feature="resumeJd")
    completed = service.create_pasted_job_description(
        user_id=user_id,
        text=request.text,
