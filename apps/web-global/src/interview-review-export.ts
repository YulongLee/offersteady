...
export const formatInterviewReviewMarkdown = (
  ...
  const lines = [
    `# ${safeLine(title)}`,
    "",
    `- Started: ${formatTime(review.startedAtMs)}`,
    `- Ended: ${formatTime(review.endedAtMs)}`,
  ...
  if (questions.length) {
    [...questions].reverse().forEach((question, index) => {
      lines.push(
        ...
        `- Source: ${question.input === "screenshot" ? "Screenshot" : question.input === "manual" ? "Manual input" : "Audio transcript"}`,
        "",
