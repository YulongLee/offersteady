...
const toSafeAccountSummary = (user: CurrentUserResponse): SafeAccountSummary => ({
  ...
  bindings: user.bindings.map(item => ({
    ...
    displayName: item.displayName ?? (item.provider === "wechat" ? "微信账号" : item.provider === "sms" ? "手机号" : item.provider === "email" ? "Email" : "账号"),
...
export const authClient = {
  ...
  async sendEmailCode(email: string, signal?: AbortSignal) {
    return client.request<EmailSendCodeResponse>("/api/v1/auth/email/send-code", {
      method: "POST",
      body: JSON.stringify({ email, clientLabel: "web-global" }),
    }, signal);
  },

  async verifyEmailLogin(input: { email: string; challengeId: string; code: string }, signal?: AbortSignal): Promise<StoredAuthSession> {
    const result = await client.request<AuthResultResponse>("/api/v1/auth/email/verify-login", {
      method: "POST",
      body: JSON.stringify({ email: input.email, challengeId: input.challengeId, code: input.code, clientLabel: "web-global" }),
    }, signal);
    return storeSession(result);
  },

  async sendGlobalEmailCode(email: string, purpose: "registration" | "password_setup" | "password_reset", signal?: AbortSignal) {
    return client.request<EmailSendCodeResponse>("/api/v1/auth/global/email/send-code", {
      method: "POST",
      body: JSON.stringify({ email, purpose, clientLabel: "web-global" }),
    }, signal);
  },

  async loginGlobal(input: { email: string; password: string }, signal?: AbortSignal): Promise<StoredAuthSession> {
    const result = await client.request<AuthResultResponse>("/api/v1/auth/global/password/login", {
      method: "POST",
      body: JSON.stringify({ ...input, clientLabel: "web-global" }),
    }, signal);
    return storeSession(result);
  },

  async completeGlobalPasswordFlow(input: { mode: "registration" | "password_setup" | "password_reset"; email: string; challengeId: string; code: string; password: string }, signal?: AbortSignal): Promise<StoredAuthSession> {
    const path = input.mode === "registration" ? "/api/v1/auth/global/register" : input.mode === "password_setup" ? "/api/v1/auth/global/password/setup" : "/api/v1/auth/global/password/reset";
    const result = await client.request<AuthResultResponse>(path, {
      method: "POST",
      body: JSON.stringify({ email: input.email, challengeId: input.challengeId, code: input.code, password: input.password, clientLabel: "web-global" }),
    }, signal);
    return storeSession(result);
  },

  async changeGlobalPassword(input: { currentPassword: string; newPassword: string }, signal?: AbortSignal): Promise<void> {
    const existing = this.readStoredSession();
    if (!existing) throw new AppError("validation", "You are not signed in.");
    await client.request<{ changed: boolean }>("/api/v1/auth/global/password/change", {
      method: "POST",
