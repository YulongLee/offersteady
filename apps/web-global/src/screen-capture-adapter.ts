...
const dataUrlToFile = (dataUrl: string, name: string) => {
  ...
  if (!meta || !payload) throw new Error("The companion did not return valid screenshot data.");
...
const canvasBlob = (canvas: HTMLCanvasElement, type = "image/png") => new Promise<Blob>((resolve, reject) => {
  canvas.toBlob(blob => {
    if (!blob) {
      reject(new Error("Screen capture failed. Please try again."));
      return;
    }
    resolve(blob);
  }, type);
...
export const screenCaptureAdapter: ScreenCaptureAdapter = {
  async captureCurrentScreen(signal) {
    if (window.offersteadyDesktop?.captureCurrentScreen) {
      ...
      if (!captured.dataUrl) throw new Error("The companion did not return a screenshot image.");
      return {
        name: captured.name?.trim() || "Current Screen Capture",
        ...(typeof captured.width === "number" ? { width: captured.width } : {}),
        ...(typeof captured.height === "number" ? { height: captured.height } : {}),
        source: "desktop-bridge",
        file: dataUrlToFile(captured.dataUrl, captured.name?.trim() || "Current Screen Capture"),
      };
    }
    ...
    if (typeof navigator !== "undefined" && navigator.mediaDevices?.getDisplayMedia) {
      ...
      try {
        ...
        if (!context) throw new Error("This browser cannot render the screenshot.");
        context.drawImage(video, 0, 0, width, height);
        const blob = await canvasBlob(canvas, "image/png");
        const name = videoTrack?.label?.trim() || "Current Screen Capture";
        return {
          name,
          width,
          height,
