...
export const toPreparationAudioReadiness = (
  ...
  const sources = requiredSources.map(sourceKind => {
    ...
    return {
      ...
      message: degraded
        ? `${label} is unavailable. Check it again in the desktop companion.`
        : ready
          ? `${label} audio check passed.`
          : health?.state === "silent"
            ? `${label} is open and waiting for audio.`
            : `${label} and transcription are preparing.`,
    };
  });
  return {
...
const runtimeNotice = (runtime: BackendRealtimeRuntimeResponse | null, degradedEvent?: BackendRealtimeEventListResponse["events"][number]) => {
  ...
  if (runtime.readinessState === "degraded") {
    const unavailable = Object.entries(runtime.sourceReadiness ?? {})
      ...
      .join("、");
    return { stage: runtime.stage, message: `${unavailable || "Live audio"} is not ready. Other available channels will continue; check the device in the desktop companion.` };
  }
...
export class BackendPreviewInterviewAdapter implements InterviewAppAdapter {
  ...
  private acknowledgeRuntimePerformance(
    ...
    delivery?: {
      ...
      readonly routeReceivedAtMs?: number;
      readonly executorAdmittedAtMs?: number;
      readonly answerGeneratorStartedAtMs?: number;
    ...
    const send = () => {
      ...
      this.enqueuePerformanceAck(key, () => this.client.request(`/api/v1/realtime-speech/sessions/${interviewId}/performance-ack`, {
        ...
        body: JSON.stringify({
          ...
          ...(delivery?.routeReceivedAtMs === undefined ? {} : { routeReceivedAtMs: delivery.routeReceivedAtMs }),
          ...(delivery?.executorAdmittedAtMs === undefined ? {} : { executorAdmittedAtMs: delivery.executorAdmittedAtMs }),
          ...(delivery?.answerGeneratorStartedAtMs === undefined ? {} : { answerGeneratorStartedAtMs: delivery.answerGeneratorStartedAtMs }),
  ...
  async createDraft(input: { title: string; role: string; company?: string; sessionMode?: SessionMode; interviewLanguage?: InterviewLanguage; interviewAudioMode?: InterviewAudioMode }, signal?: AbortSignal) {
    ...
    const created = await this.client.request<BackendSessionResponse>("/api/v1/sessions", {
      ...
      body: JSON.stringify({
        ...
        interviewLanguage: input.interviewLanguage ?? "en-US",
  ...
  async subscribeRealtimeSession(interviewId: string, onUpdate: Parameters<InterviewAppAdapter["subscribeRealtimeSession"]>[1], signal?: AbortSignal, lease?: { readonly pageInstanceId: string; readonly leaseGeneration: number }) {
    ...
    if (!response.ok) {
      const error = new AppError("validation", `Live conversation subscription failed (${response.status}).`) as AppError & {
        status: number;
      };
  ...
  private async submitManualAnswerStream(command: Parameters<InterviewAppAdapter["submitManualAnswer"]>[0], signal: AbortSignal | undefined, onStreamUpdate: (update: ManualAnswerStreamUpdate) => void): Promise<SubmitManualAnswerResult> {
    ...
    if (!response.ok) throw new AppError("validation", `Unable to start the live answer (${response.status}).`);
    ...
    try {
      ...
      while (Date.now() < deadlineAt) {
        ...
        if (pushed) {
          ...
          if (pushedStatus === "failed") throw new AppError("validation", `${pushed.stage ? `Screenshot stage ${String(pushed.stage)} failed: ` : ""}${String(pushed.errorMessage || "The desktop companion could not create the screenshot answer. Check its status and try again.")}`);
        ...
        if (current.status === "failed") throw new AppError("validation", `${current.stage ? `Screenshot stage ${current.stage} failed: ` : ""}${current.errorMessage || "The desktop companion could not create the screenshot answer. Check its status and try again."}`);
