...
export const normalizeError = (error: unknown): AppError => {
  ...
  if (error instanceof DOMException && error.name === "AbortError") return new AppError("aborted", "Request cancelled");
  if (typeof error === "object" && error !== null && "status" in error && (error as { status?: number }).status === 501) return new AppError("not-implemented", "This feature is not enabled in the current environment");
  if (error instanceof Error && error.message) return new AppError("unknown", error.message);
  return new AppError("unknown", "The action could not be completed. Please try again.");
};
