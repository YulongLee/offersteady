...
function NewInterviewPage() {
  ...
  return <main className="app-page narrow"><Link className="back-link" to={routes.app}>← 返回面试首页</Link><PageHeader eyebrow="NEW INTERVIEW" title="创建一场面试" detail="Choose the interview device first, then add the role and session details." /><form className="form-panel" onSubmit={submit}><fieldset className="interview-audio-mode-picker"><legend>Interview device</legend><p>Computer interviews use two audio channels. Mobile interviews listen to your phone speaker through the Mac microphone.</p><div><label className={form.interviewAudioMode === "computer" ? "selected" : ""}><input type="radio" name="interview-audio-mode" value="computer" checked={form.interviewAudioMode === "computer"} onChange={() => setForm({ ...form, interviewAudioMode: "computer" })} /><span><strong>Computer interview</strong><small>The interview runs on this Mac with separate interviewer and candidate audio.</small></span></label><label className={form.interviewAudioMode === "mobile" ? "selected" : ""}><input type="radio" name="interview-audio-mode" value="mobile" checked={form.interviewAudioMode === "mobile"} onChange={() => setForm({ ...form, interviewAudioMode: "mobile" })} /><span><strong>Mobile interview</strong><small>Use phone speaker mode while the Mac microphone listens to room audio.</small></span></label></div></fieldset><label>面试名称<input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="例如：高级前端工程师一面" /></label><label>目标岗位<input value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} placeholder="例如：高级前端工程师" /></label><label>公司（可选）<input value={form.company} onChange={e => setForm({ ...form, company: e.target.value })} placeholder="例如：示例科技" /></label>{error ? <div className="inline-error" role="alert">{error}</div> : null}<div className="form-actions"><Link className="button ghost" to={routes.app}>取消</Link><button className="button primary" type="submit" disabled={saving}>{saving ? "创建中…" : "保存并准备 →"}</button></div><small className="saved-note">草稿只在你确认提交后保存。</small></form></main>;
}
...
function PreparationPage() {
  ...
  const inputDiagnostic = machineReady
      ? isMobileInterview
      ? "The companion is connected and preparing the Mac microphone and live transcription."
      : "伴随程序已连接，正在后台准备音频与实时识别"
      : state.preparation.device?.connected
      ? isMobileInterview
      ? "The companion will check the Mac microphone and question detection."
  ...
  return <main className="app-page"><Link className="back-link" to={routes.app}>← 返回面试首页</Link><PageHeader eyebrow="PREPARATION" title={interviewTitle} detail="资料与“面试资料”页面保持一致，为本场按需选择。" action={<div className="completion"><strong>{complete}/2</strong><span>{canStart ? "可进入" : "准备中"}</span></div>} />
    {isMobileInterview ? <div className="mobile-interview-guidance" role="note"><span>MOBILE INTERVIEW</span><strong>Let the Mac microphone hear the phone speaker clearly</strong><ul><li>Turn on speaker mode and place the phone near the Mac microphone.</li><li>Do not use headphones, or the Mac will not hear the interviewer.</li><li>Keep the room quiet. Mixed transcripts are shown as “Room audio”.</li></ul></div> : null}
...
function LivePage() {
  ...
  const interviewAudioMode = liveInterview?.interviewAudioMode ?? "computer";
  ...
  const conversationPanel = <ConversationMonitor state={state} interviewAudioMode={interviewAudioMode} onConfirmQuestion={pageLeaseStatus === "replaced" ? dismissPending : confirmPending} onDismissQuestion={dismissPending} />;
...
function ReviewPage() {
  ...
  const interview = state.interviews.find(item => item.id === id);
  const isWritten = interview?.sessionMode === "written";
  const reviewAudioMode = state.review.interviewAudioMode ?? interview?.interviewAudioMode ?? "computer";
