...
export const contextSourceStatusLabel: Record<ContextLibrarySource["status"], string> = {
...
export const displayedContextSourceStatus = (source: ContextLibrarySource) =>
  source.status === "failed" && source.summary?.startsWith("Upload failed")
    ? "Upload failed"
