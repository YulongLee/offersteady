...
describe("local companion 1.3.2 release boundary", () => {
  it("keeps the desktop package and workspace lockfile aligned at 1.3.2", () => {
    const packageJson = JSON.parse(readFileSync(new URL("../package.json", import.meta.url), "utf8"));
    const lockfile = JSON.parse(readFileSync(new URL("../../../package-lock.json", import.meta.url), "utf8"));

    expect(packageJson.version).toBe("1.3.2");
    expect(lockfile.packages["apps/desktop"].version).toBe("1.3.2");
  });
