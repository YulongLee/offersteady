...
export function AnswerWorkspace({ answers, viewingAnswerId, newAnswerAvailable, activeTask, cancelling, cancelError, onView, onRetry, onStop }: Props) {
  ...
  return <section className={`answer-workspace${mobileExpanded ? " mobile-answer-expanded" : ""}`} aria-labelledby="answer-title" aria-live="polite">
    ...
    {shown.status !== "cancelled" ? <div className="advice-card"><div className="advice-heading"><span className="advice-label">AI 回答</span><span className="confidence">{shown.advice.uncertain ? "资料较少" : "可直接参考"}</span></div><div className="answer-body structured-answer" aria-label="回答正文"><section className="answer-section simple-answer"><div className="answer-section-title"><span>简单回答</span><small>先说这段</small></div><AnswerMarkdown content={answerSections.simple} /></section>{answerSections.detailed || taskActive ? <section className="answer-section detailed-answer"><div className="answer-section-title"><span>详细回答</span><small>{retrievedCount > 0 ? `已引用知识库 ${retrievedCount}` : taskActive ? "正在补充" : "未引用知识库"}</small></div>{answerSections.detailed ? <AnswerMarkdown content={answerSections.detailed} /> : <p className="answer-placeholder">正在结合资料补充详细回答…</p>}</section> : null}</div>{unavailableSources.length ? <div className="context-warning" role="status">有 {unavailableSources.length} 份已选资料本次未使用：{unavailableSources.map(source => source.displayName).join("、")}。请回到资料库重新处理后再确认本场资料。</div> : null}<div className="advice-footer"><div className="source-pills"><small>{minimalSourceSummary}</small>{shown.advice.provenance.usedSources.map(source => <span key={`${source.sourceId}-${source.contextRole ?? "source"}`}>{source.contextRole === "retrieved" ? "知识库" : source.kind === "jd" ? "JD" : "固定"} · {source.displayName} {source.sourceVersion}{source.truncated ? " · 已截断" : ""}</span>)}</div><span>{shown.advice.uncertain ? "资料不足时请只使用你能核对的真实经历" : "请按真实经历表达，避免补造细节"}</span></div></div> : null}
  </section>;
}
