...
export function AnswerActionBar({ manualDraft, latestInterviewerQuestion = "", screenshotTask, onQuickAnswer, onScreenshot, disabled = false, quickAnswerStatus = "idle", quickAnswerMessage = "", screenshotAnswerStatus = "idle" }: Props) {
  ...
  const quickStatusText = quickBusy ? "正在生成回答，请稍候" : quickAnswerStatus === "success" ? "快答已完成" : quickAnswerStatus === "failed" ? quickAnswerMessage || "快答失败，可重试" : quickAnswerStatus === "cancelled" ? "快答已终止，可重新发起" : "";
  const screenshotStatusText = screenshotAnswerStatus === "success" ? "截屏回答已完成，答案已显示" : screenshotAnswerStatus === "cancelled" ? "截屏回答已取消" : screenshotStatus(screenshotTask);
  return <section className="answer-action-bar" aria-label="面试操作">
    <div className="answer-action-buttons">
      ...
      >
        <strong>{quickBusy ? "快答中…" : quickAnswerStatus === "success" ? "已回答" : "快答"}</strong>
        <small>{quickBusy ? "问题已收到，正在生成答案" : manualDraft.trim() ? "根据左侧问题直接生成回答" : "根据最近面试官问题回答"}</small>
      </button>
      <button
