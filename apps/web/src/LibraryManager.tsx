...
> = {
  resume: {
    title: "简历",
    detail: `维护可复用简历；具体使用哪一份，在每场面试准备时选择。${supportedFormatsLabel}。`,
    action: "＋ 添加简历",
  },
  jd: {
    title: "职位 JD",
    detail: `通过粘贴文本或文件保存目标岗位；不会自动加入任何面试。${supportedFormatsLabel}，也支持直接粘贴 JD 文本。`,
    action: "＋ 添加 JD",
  },
  knowledge: {
    title: "知识库",
    detail: `按主题管理知识材料，每场面试再选择需要的内容。${supportedFormatsLabel}。`,
    action: "＋ 新建资料库",
  },
};
...
export function LibraryManager({ state, setState }: Props) {
    ...
    try {
      ...
      setError(
        reason instanceof Error
          ? `${reason.message}；你可以重新上传文件，或通过使用手册中的客服入口联系我们。`
          : "重新处理失败，请重新上传文件或联系客服。",
      );
    } finally {
      setOperation(null);
  ...
  const uploadKnowledge = async () => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : "上传知识资料失败，请稍后重试",
      );
    } finally {
  ...
  const addSource = async () => {
    ...
    setPendingFile(null);
    setNotice(
      `${materialTab === "resume" ? "简历" : "JD"}正在后台上传并解析，你可以继续使用其他功能`,
    );
    const optimisticId = `uploading-${crypto.randomUUID()}`;
    const optimisticNow = Date.now();
    const optimisticSource: ContextLibrarySource = {
    ...
    try {
      ...
      setNotice(
        `${materialTab === "resume" ? "简历" : "JD"}已上传，等待服务端处理；尚未授权给任何面试`,
      );
    } catch (error) {
      setState((current) => ({
        ...current,
  ...
  const removeSource = (source: ContextLibrarySource) => {
    if (
      !window.confirm(`删除“${source.displayName}”？它将立即停止参与后续回答。`)
    )
      return;
  ...
  const removeDocument = (document: KnowledgeDocumentVersion) => {
    if (
      !window.confirm(
        `删除“${document.displayName}”？它将立即停止参与后续回答，历史回答仅保留名称和版本。`,
      )
    )
      return;
  ...
  const removeCollection = () => {
    if (
      !selected ||
      !window.confirm(
        `删除资料库“${selected.name}”及其中 ${documents.length} 份资料？`,
      )
    )
      return;
  ...
  const updateDocument = (
    ...
    const nextName =
      action === "rename"
        ? window.prompt("新的资料名称", document.displayName)?.trim()
        : "";
  ...
  return (
    <main className="app-page">
      ...
      <nav className="material-tabs" aria-label="资料类型">
        {(["resume", "jd", "knowledge"] as const).map((item) => (
          ...
          >
            ...
            <small>
              {item === "knowledge"
                ? `${state.knowledgeDocuments.filter((doc) => doc.status !== "deleted").length} 份`
                : `${state.librarySources.filter((source) => source.kind === item && source.status !== "deleted").length} 份`}
            </small>
          </button>
        ))}
      </nav>
            ...
            <div className="panel-heading">
              ...
              <span>
                {operation === "create-collection"
                  ? "创建中"
                  : `${state.knowledgeCollections.length} 个`}
              </span>
            </div>
            ...
            {selected ? (
              <>
                ...
                {documents.length ? (
                  ...
                  <EmptyMaterial
                    title="这个知识库还是空的"
                    detail={`添加并成功索引文件后才结算；会员额度适用于知识材料，不是空集合。${supportedFormatsLabel}。`}
                    action="添加第一份资料"
              ...
              <EmptyMaterial
                title="创建你的第一个知识库"
                detail={`可按岗位、技术方向或项目经历分类。${supportedFormatsLabel}。`}
                action="新建资料库"
        ...
        <section className="panel typed-material-panel">
          <div className="panel-heading">
            <div>
              ...
              <small>
                {tab === "jd"
                  ? `${supportedFormatsLabel}，也支持直接粘贴 JD 文本。`
          ...
          {tabSources.length ? (
            <div className="typed-material-list">
              {tabSources.map((source) => (
                <article key={source.id}>
                  ...
                  <div>
                    <span
                      className={`state-mark ${source.status === "ready" && source.syncStatus !== "missing_artifacts" ? "ready" : source.status === "processing" ? "processing" : "error"}`}
                    >
            ...
            <EmptyMaterial
              title={`还没有${tabCopy[tab].title}`}
              detail={
                tab === "resume"
                  ? `添加后可在不同面试中选择使用。${supportedFormatsLabel}。`
                  : `支持上传 ${supportedFormatsLabel.replace("支持上传 ", "")}，也支持粘贴 JD 文本。`
              }
              action={tabCopy[tab].action.replace("＋ ", "")}
      ...
      {dialog === "add-source" && tab !== "knowledge" ? (
        <DialogShell
          title={tab === "resume" ? "添加简历" : "添加职位 JD"}
          onClose={() => setDialog(null)}
        >
          <p>
            {tab === "resume"
              ? `${supportedFormatsLabel}。资料只加入可选列表，不会自动用于任何面试。`
              : `${supportedFormatsLabel}，也支持直接粘贴 JD 文本。资料只加入可选列表，不会自动用于任何面试。`}
          </p>
          <label className="checkout-field">
            显示名称（可选）
            <input
          ...
          <label className="checkout-field">
            选择文件
            <input
              aria-label={tab === "resume" ? "选择简历文件" : "选择 JD 文件"}
              type="file"
              accept={materialUploadAccept}
              disabled={submittingUpload}
              onChange={(event) =>
