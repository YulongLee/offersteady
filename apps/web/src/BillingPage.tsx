...
export function BillingPage({ state, setState }: Props) {
  ...
  const startCheckout = async (selectedProduct: BillingProduct) => {
    ...
    try {
      ...
      setNotice(order.provider === "alipay" ? "支付宝官方订单已创建，已为你打开官方收银台；到账以服务端验签通知为准。" : "支付订单已创建，已为你打开聚合收银台；到账以服务端验签通知为准。");
    } catch (error) {
  ...
  return <main className="app-page billing-page"><header className="billing-hero"><div><span className="kicker">PRICING & POINTS · 目录 v{state.billing.rates.catalogVersion}</span><h1>按你的面试节奏付费</h1><p>新用户赠送 200 点体验额度。低频按点，高频选择按天会员；商品与金额由服务端确认，到账以后端验签结果为准。</p></div><div className="balance-card"><small>{state.billing.activePass ? "会员有效期内回答与截图不限次" : "当前可用"}</small><strong>{state.billing.balance} 点</strong><span>回答 {state.billing.rates.answerPoints} 点 · 截图 {state.billing.rates.screenshotAnswerPoints} 点 · 知识材料 {state.billing.rates.knowledgeIndexMinimumPoints} 点起</span>{state.billing.activePass ? <span>知识材料额度 {allowance}/{state.billing.activePass.knowledgeAllowanceGranted} · {new Date(state.billing.activePass.endsAtMs).toLocaleDateString("zh-CN")} 到期</span> : null}</div></header>{notice ? <div className="billing-notice" role="status">{notice}</div> : null}
    ...
    {product ? <div className="sheet-backdrop" role="dialog" aria-modal="true" aria-labelledby="official-checkout-title"><section className="sheet checkout-sheet"><button className="sheet-close" aria-label="关闭支付" onClick={() => setProduct(null)}>×</button><h2 id="official-checkout-title">购买 {product.displayName}</h2><p>订单金额由服务端目录确认：<strong>{money(product.priceCents)}</strong></p>{checkoutPending && !checkout ? <div className="payment-waiting"><i className="online-dot" /><span>正在创建安全支付订单并跳转收银台…</span></div> : null}{checkout ? <><div className="official-order-meta"><span>订单号</span><strong>{checkout.id}</strong><span>金额</span><strong>{money(checkout.amountCents)}</strong><span>状态</span><strong>{officialStatus[checkout.status]}</strong></div>{checkout.status === "payment_pending" ? checkout.action.kind === "dynamic_qr" ? <div className="dynamic-payment-qr"><span>订单专属动态支付二维码</span><small>5 分钟内有效 · 不属于静态图片资产</small></div> : <a className="button primary full" href={checkout.action.url} target="_blank" rel="noreferrer">{checkout.provider === "alipay" ? "打开支付宝官方收银台" : "打开支付收银台"}</a> : null}{checkout.status === "payment_pending" ? <div className="payment-waiting"><i className="online-dot" /><span>正在等待服务端验签通知，请勿重复付款</span></div> : null}</> : null}</section></div> : null}
  </main>;
}
