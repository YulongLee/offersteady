...
function LivePage() {
  ...
  const captureStatus = pageLeaseStatus === "replaced" ? "已在其他页面继续" : state.captureState === "capturing" ? "正在收音" : state.captureState === "paused" ? "收音已暂停" : state.captureState === "reconnecting" ? "设备正在重连" : state.captureState === "permission-required" ? "采集能力不可用" : state.captureState === "error" ? "设备连接异常" : "已连接，未采集";
  const captureButton = state.captureState === "capturing"
  ...
  return <main className={`live-page focused-live-page${desktopLayout ? " desktop-live-page" : " mobile-live-page"}`}>
    ...
    {desktopLayout ? <footer className="session-bar"><div><i className={state.captureState === "capturing" ? "recording-dot" : "online-dot"} /><strong>{state.captureState === "capturing" ? "面试进行中" : state.captureState === "paused" ? "面试已暂停" : "等待开始面试"}</strong></div><div><small>{state.captureState === "capturing" ? "正在持续接收面试官与我的实时对话" : "开始面试后会在头部右侧管理本场状态"}</small></div></footer> : null}
  </main>;
}
