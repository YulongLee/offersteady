  ...
  return (
    <div className="public-shell">
      <header className="public-nav">
        ...
        <nav aria-label="公开导航"><Link to={`${routes.landing}#workflow`}>使用方式</Link><a href="/pricing">积分与会员</a><a href="/download">下载助手</a><Link to={routes.privacy}>隐私政策</Link><Link className="button ghost" to={authenticated ? routes.app : routes.login}>{authenticated ? "进入应用" : "登录"}</Link></nav>
      </header>
      <Outlet />
    </div>
  );
...
function LandingPage() {
  ...
  return (
    <main>
      ...
      <footer className="public-footer"><div className="public-footer-main"><section className="footer-brand"><Logo /><p>面向求职者的 AI 面试辅助工具，从资料准备、现场表达建议到面试复盘，让每一次面试更有条理。</p><span>AI 输出仅供参考，请始终以真实经历作答。</span></section><nav className="footer-column" aria-label="页脚产品导航"><h2>产品</h2><a href="/features/ai-interview-assistant">AI 面试助手</a><a href="/features/realtime-interview">实时面试辅助</a><a href="/features/screenshot-answer">截图回答</a><a href="/features/interview-review">面试复盘</a><a href="/pricing">积分与会员</a><a href="/download">下载助手</a></nav><nav className="footer-column" aria-label="页脚资源导航"><h2>资源</h2><Link to={routes.publicGuide}>使用手册</Link><Link to={`${routes.publicGuide}#desktop`}>下载安装说明</Link><a href="/guides/interview-preparation">面试准备清单</a><a href="/guides/star-interview-answer">STAR 回答结构</a><a href="/guides/audio-troubleshooting">收音问题排查</a><a href="/security">安全说明</a><a href="/about">关于产品</a><a href="/contact">联系我们</a><Link to={routes.terms}>用户协议</Link><Link to={routes.privacy}>隐私政策</Link></nav><section className="footer-contact"><h2>联系我们</h2><dl><div><dt>客服微信</dt><dd>{state.billing.support.wechatId || "暂未配置"}</dd></div><div><dt>联系邮箱</dt><dd>{state.billing.support.email ? <a href={`mailto:${state.billing.support.email}`}>{state.billing.support.email}</a> : "暂未配置"}</dd></div><div><dt>服务时间</dt><dd>{state.billing.support.serviceHours || "请通过邮箱留言"}</dd></div></dl><p>咨询订单时请提供订单号，请勿发送密码、验证码或完整身份资料。</p></section></div><div className="public-footer-legal"><span>© 2026 面试稳AI助手 · OneShow AI Lab</span><a href="https://beian.miit.gov.cn" target="_blank" rel="noreferrer">浙ICP备2026052190号-1</a></div></footer>
    </main>
  );
}
...
function LivePage() {
  ...
  return <main className="live-page focused-live-page"><header className="live-top"><Link to={routes.app}><Logo /></Link><div><strong>{interviewTitle}</strong><span><i className={state.captureState === "capturing" ? "recording-dot" : "online-dot"} /> {pageLeaseStatus === "replaced" ? "已在其他页面继续" : state.captureState === "capturing" ? "这台设备 · 正在收音" : state.captureState === "paused" ? "收音已暂停" : state.captureState === "reconnecting" ? "设备正在重连" : state.captureState === "permission-required" ? "助手采集能力不可用" : state.captureState === "error" ? "设备连接异常" : "这台设备 · 已连接，未采集"}</span></div><div className="live-top-actions"><Link className="live-balance" to={routes.billing}>积分与会员</Link><span>18:24</span><AccountMenu compact />{state.captureState === "capturing" ? <button className="button warning live-session-control" disabled={pageLeaseStatus === "replaced" || captureControlPending !== null} onClick={() => void controlCapture("pause")}>{captureControlPending === "pause" ? "暂停中…" : "暂停收音"}</button> : <button className="button primary live-session-control" disabled={pageLeaseStatus === "replaced" || captureControlPending !== null || (state.captureState !== "ready" && state.captureState !== "paused")} onClick={() => state.captureState === "paused" ? void controlCapture("resume") : setCapture("capturing", "active")}>{captureControlPending === "resume" ? "恢复中…" : state.captureState === "paused" ? "恢复收音" : "开始面试"}</button>}<button className="button danger live-session-control" disabled={pageLeaseStatus === "replaced"} onClick={() => void finishInterview()}>结束面试</button></div></header>
    ...
    {notice ? <div className="global-live-alert" role="status"><strong>{notice}</strong><span>{billingNotice ? "当前任务未启动，请检查积分或会员权益。" : "当前回答没有成功启动，请根据上方原因重试。"}</span>{billingNotice ? <Link className="button primary" to={routes.billing}>前往积分与会员</Link> : null}</div> : null}
