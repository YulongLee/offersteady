...
export const syntheticState: WebAppState = {
  ...
  billing: {
    ...
    orders: [],
    officialOrders: [],
    support: { wechatId: "mianshiwen-cn", email: "contact@oneshowailab.com", qrAssetPath: "", serviceHours: "工作日 10:00–18:00", refundSummary: "退款按订单状态和未使用权益人工审核" },
  },
  speaker: {
    mode: "dual-channel",
    transcripts,
...
export class FixtureInterviewAdapter implements InterviewAppAdapter {
  ...
  async subscribeRealtimeSession(_interviewId: string, onUpdate: Parameters<InterviewAppAdapter["subscribeRealtimeSession"]>[1], signal?: AbortSignal, lease?: { readonly pageInstanceId: string; readonly leaseGeneration: number; readonly onTransportConnected?: () => void }) {
    await delay(signal);
