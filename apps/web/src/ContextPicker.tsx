...
export function ContextPicker({ sources, selection, onSave, onCancel, onDownload }: ContextPickerProps) {
  ...
  const group = (title: string, items: readonly ContextLibrarySource[], kind: "resume" | "jd") => <fieldset className="context-group"><legend>{title} <small>可选 · 单选</small></legend><label className={`context-option context-none ${!(kind === "resume" ? resumeSourceId : jobDescriptionSourceId) ? "selected" : ""}`}><input type="radio" name={`${selection.sessionId}-${kind}`} checked={!(kind === "resume" ? resumeSourceId : jobDescriptionSourceId)} onChange={() => kind === "resume" ? setResume(null) : setJd(null)} /><span><strong>本场不使用{title}</strong><small>仍可开始面试，回答不会引用这类资料</small></span></label>{items.map((source, index) => {
    ...
    return <label className={`context-option ${checked ? "selected" : ""} ${disabled ? "disabled" : ""}`} key={source.id}><input type="radio" name={`${selection.sessionId}-${kind}`} checked={checked} disabled={disabled} onChange={() => kind === "resume" ? setResume(source.id) : setJd(source.id)} /><span><strong>{source.displayName}</strong><small>{source.version} · {dateLabel(source.updatedAtMs)} · {contextSourceStatusLabel[source.status]}{kind === "resume" && index === 0 ? " · 最近使用建议" : ""}</small></span>{onDownload ? <button type="button" className="context-download" onClick={event => { event.preventDefault(); event.stopPropagation(); void onDownload(source); }} aria-label={`下载 ${source.displayName}`}>下载</button> : null}{checked ? <b>✓</b> : null}</label>;
  })}</fieldset>;
