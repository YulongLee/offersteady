...
import { App } from "./App";
import { parseReferralActivationInput } from "./BillingPage";
...
describe("device center and referral growth", () => {
  it.each([
    ...
    ["  https://mianshiwen.cn/invite/syntheticReferralCode  ", "syntheticReferralCode"],
  ...
  it("activates a pasted referral link from billing and refreshes authoritative status", async () => {
    const initialStatus = {
      enabled: true, rewardPoints: 500, configVersion: 2, referralCode: "currentUsersReferralCode",
      shareUrl: "https://example.test/invite/currentUsersReferralCode", inviteCount: 0,
      totalRewardPoints: 0, hasActivatedReferral: false,
    };
    const getReferralStatus = vi.spyOn(interviewAppAdapter, "getReferralStatus")
      .mockResolvedValueOnce(initialStatus)
      .mockResolvedValue({ ...initialStatus, hasActivatedReferral: true });
    const activate = vi.spyOn(interviewAppAdapter, "activateReferral").mockResolvedValue({
      outcome: "activated", replayed: false, rewardPoints: 500, activatedAtMs: Date.now(),
    });
    open("/app/billing");

    const input = await screen.findByLabelText("邀请链接或邀请码");
    fireEvent.change(input, { target: { value: "https://mianshiwen.cn/invite/friendsReferralCode" } });
    fireEvent.click(screen.getByRole("button", { name: "确认激活" }));

    await waitFor(() => expect(activate).toHaveBeenCalledWith("friendsReferralCode", expect.any(AbortSignal)));
    expect(await screen.findByText("邀请已成功激活，奖励已发放给邀请人。")).toBeInTheDocument();
    expect(await screen.findByText("已激活过邀请，不能再次激活其他链接。")).toBeInTheDocument();
  ...
  it("rejects malformed input locally and displays server self-referral feedback", async () => {
    ...
    const activate = vi.spyOn(interviewAppAdapter, "activateReferral").mockResolvedValue({ outcome: "self-referral" });
    open("/app/billing");

    const input = await screen.findByLabelText("邀请链接或邀请码");
    fireEvent.change(input, { target: { value: "bad" } });
    fireEvent.click(screen.getByRole("button", { name: "确认激活" }));
    expect(await screen.findByText("请输入有效的邀请链接或邀请码。")).toBeInTheDocument();
    expect(activate).not.toHaveBeenCalled();

    fireEvent.change(input, { target: { value: "currentUsersReferralCode" } });
    fireEvent.click(screen.getByRole("button", { name: "确认激活" }));
    expect(await screen.findByText("不能激活自己的邀请链接。")).toBeInTheDocument();
  });
  ...
  it("does not render an activation form after the account has activated once", async () => {
    vi.spyOn(interviewAppAdapter, "getReferralStatus").mockResolvedValue({
      enabled: true, rewardPoints: 500, configVersion: 2, referralCode: "currentUsersReferralCode",
      shareUrl: "https://example.test/invite/currentUsersReferralCode", inviteCount: 0,
      totalRewardPoints: 0, hasActivatedReferral: true,
    });
    open("/app/billing");
    expect(await screen.findByText("已激活过邀请，不能再次激活其他链接。")).toBeInTheDocument();
