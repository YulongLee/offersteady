...
export const adminApi = {
  ...
  globalCommerceOverview: (_mode: "test" | "live" = "live") => request<Record<string, unknown>>(`/api/v1/admin/global-commerce/overview?mode=live`),
  globalCommerceOperations: (_mode: "test" | "live" = "live") => request<Record<string, unknown>>(`/api/v1/admin/global-commerce/operations?mode=live`),
  saveGlobalCommerceCredentials: (_mode: "test" | "live", apiKey: string, webhookSecret: string, reason: string) => request<Record<string, unknown>>(`/api/v1/admin/global-commerce/credentials?mode=live`, { method: "PUT", body: JSON.stringify({ apiKey: apiKey || null, webhookSecret: webhookSecret || null, reason }) }),
  globalCommerceProducts: (_mode: "test" | "live") => request<{ mode: string; items: Record<string, unknown>[]; syncedAtMs: number }>(`/api/v1/admin/global-commerce/products?mode=live`),
  saveGlobalCommerceMapping: (_mode: "test" | "live", offerCode: string, providerProductId: string, reason: string) => request<Record<string, unknown>>(`/api/v1/admin/global-commerce/mappings/${encodeURIComponent(offerCode)}?mode=live`, { method: "PUT", body: JSON.stringify({ providerProductId, reason }) }),
  activateGlobalCommerce: (_mode: "test" | "live", enabled: boolean, reason: string) => request<Record<string, unknown>>(`/api/v1/admin/global-commerce/activation?mode=live`, { method: "POST", body: JSON.stringify({ enabled, confirmed: true, reason }) }),
  ...
  baiduRanking: () => request<Record<string, unknown>>("/api/v1/admin/seo/baidu-ranking"),
  refreshBaiduRanking: (reason: string) => request<Record<string, unknown>>("/api/v1/admin/seo/baidu-ranking/refresh", { method: "POST", body: JSON.stringify({ reason }) }),
  addBaiduKeyword: (keyword: string, reason: string) => request<Record<string, unknown>>("/api/v1/admin/seo/baidu-ranking/keywords", { method: "POST", body: JSON.stringify({ keyword, reason, active: true }) }),
  updateBaiduKeyword: (keywordId: string, active: boolean, reason: string) => request<Record<string, unknown>>(`/api/v1/admin/seo/baidu-ranking/keywords/${encodeURIComponent(keywordId)}`, { method: "PATCH", body: JSON.stringify({ keyword: "placeholder", active, reason }) }),
